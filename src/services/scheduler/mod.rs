...
use alloc::format;
use spin::Mutex;
use crate::kernel::sched::task_thread::{MACH_THREADS, MACH_TASKS, ThreadState};
        ...
        if let Some(thread) = threads.get_mut(&next_id) {
            ...
            *current_thread_id_opt = Some(next_id);
            ...
            if thread.kernel_stack_top != 0 {
                arch::x86_64::update_tss_stack(thread.kernel_stack_top);
            }
            ...
            arch::x86_64::switch_address_space(vm_map_id);
            
            // Restore CPU Context
            // In a real execution, we would call an assembly wrapper:
            // arch::x86_64::switch_context(&mut old_thread.context, &thread.context);
        }
    } else {
