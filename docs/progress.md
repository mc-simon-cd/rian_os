...
- [x] **2. Gelişmiş Unix Sinyalleri**: `SIGINT`, `SIGKILL`, `SIGSEGV` trap frameleri manüpüle ederek `trap.rs` altına implement edildi.
...
- [x] **4. RAMFS (In-Memory Filesystem)**: 10MB boyut kotalı ram belleğinde saklanan `/tmp` modülü kernela kazandırıldı.
...
- [x] **6. Güvenlik Maskeleri (Capabilities)**: Cihaz dosyalarına (`/dev/kmem`) özel erişim yetkisi (`CAP_SYS_RAWIO`) getirilerek Security Subsystem modüle bağlandı.
...
- [x] **8. Gerçek Page Table Mapping (PML4)**: `copy_from_user` modülleri donanımsal mantıkla map eden memory pointer doğrulama kodlarına (map_page/unmap_page) çevrildi.
- [x] **9. Plan 9 (9P) Filesystem Network Protokolü**: Kök sanal dosya sistemi için RPC tabanlı Plan 9 (9P2000) Client, Codec ve Session altyapısı eklendi.
...
- [x] **no_std & Bare-Metal Migration**: Kernel tamamen `std` bağımlılığından arındırıldı ve `x86_64-unknown-none` hedefinde çalışır hale getirildi.
